...
npx playwright test tests/e2e/frontend.spec.ts tests/e2e/auth.spec.ts tests/e2e/whatsapp.spec.ts --project=chromium --reporter=line
if [ $? -ne 0 ]; then
...
ssh_cmd "cd $VPS_DIR && npm install --legacy-peer-deps --silent && npm run build"
